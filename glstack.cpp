...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        modelViewStack.push();
        modelViewStack.rotY(0.1f * time);            // Earth orbit rotation
        ...
        //Planet 2--------------------------------------
        modelViewStack.push();
        modelViewStack.rotY(0.6f * time);            // Earth orbit rotation
        modelViewStack.translate(0.6f, 0.0f, 0.0f);  // Earth orbit radius
        modelViewStack.push();                       // Save the matrix before the Earth's rotation
        ...
        modelViewStack.translate(1.1f, 0.0f, 0.0f);  // Earth orbit radius
        modelViewStack.push();                       // Save the matrix before the Earth's rotation
        ...
        // Moon
        modelViewStack.rotY(0.2f * time);            // Earth orbit rotation
        modelViewStack.translate(0.3f, 0.0f, 0.0f);  // Earth orbit radius
        modelViewStack.push();                       // Save the matrix before the Earth's rotation

        modelViewStack.rotY(2.0f * time);  // Earth's rotation around its axis
        modelViewStack.rotX(
            static_cast<float>(-M_PI_2));  // Orient the poles along Y axis instead of Z
        modelViewStack.scale(0.03f);        // Scale unit sphere to radius 0.1
        // Update the transformation matrix in the shader
        glUniformMatrix4fv(location_MV, 1, GL_FALSE, modelViewStack.getCurrentMatrix());
        // Render the geometry to draw the earth
